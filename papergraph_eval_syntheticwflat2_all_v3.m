figure('Position',[1429,-209,2646,1089])
Tbig = tiledlayout(2,1);

%% for flat
Tsmall = tiledlayout(Tbig,1,6);
...
%line([156 176],[208 208],'LineWidth',2,'Color','white')
...
for i=40:80
    filename = sprintf("From DL PC/svd_filter_for_orpam/complete_comparison_flat2/flat_eval_%d_1.mat",i);
    %filename = sprintf("From DL PC/svd_filter_for_orpam/complete_comparison_flat2/flat_eval_%d_1.mat",i);
    load(filename)
    % load metrics
...
%title(Tsmall,"Evaluation of USAF-1951","FontName","Times New Roman","FontSize",20,"FontWeight","bold");
...
B = std(snr_dat);
fprintf("%.2f (%.2f)\n",A(2),B(2))
%title(Tsmall,"Evaluation of Red-stained Spherical Phantom","FontName","Times New Roman","FontSize",20,"FontWeight","bold");


exportgraphics(gcf,'./exported_images/eval_synthetic_wflat2_v3.emf')